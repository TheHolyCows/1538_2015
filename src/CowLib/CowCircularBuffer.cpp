...
namespace CowLib
{
	...
	uint32_t CowCircularBuffer::GetBuffer(void *buf, uint32_t size)
	{
		...
		{
			memcpy(buf, &m_Buffer[m_StartIndex], m_Size - m_StartIndex);
			memcpy(&buf[m_Size - m_StartIndex], m_Buffer, numToGet - (m_Size - m_StartIndex));
		}
		else
		{
	...
	uint32_t CowCircularBuffer::PutBuffer(void *buf, uint32_t size)
	{
		...
		if (bottom)
		{
			memcpy(m_Buffer, &buf[top], bottom);
			m_EndIndex = bottom;
		}
